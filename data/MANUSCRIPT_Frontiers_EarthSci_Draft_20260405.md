...
**Site effects.** The absence of site characterization parameters (V_S30, Z_2.5) is the dominant factor limiting short-period Sa predictions (T = 0.3–0.6 s, R² ≈ 0.54), as demonstrated by the 23% R² gap between short and long periods (Section 3.5). A systematic V_S30 integration experiment confirmed this hypothesis in two phases. In Phase 1, retraining all 515 models with 44 features (42 P-wave IMs + V_S30 + ln V_S30) using proxy-derived V_S30 for 203 stations (~27% measured, ~73% estimated) yielded a mean ΔR² = +0.030 (+5.1%) across all 103 spectral periods without exception. In Phase 2, the V_S30 dataset was expanded to 338 BMKG stations with field-measured values (39.6% of the network), updating 265 stations (117,965 traces, 61.3% of training data) from proxy estimates to measured values. This expanded experiment produced dramatically larger improvements: mean ΔR² = +0.143 (+24.6%) over baseline, with 100% of periods (103/103) improving. Short-period performance (T = 0.05–0.6 s) improved by ΔR² = +0.194 over baseline, while long-period performance (T ≥ 3.0 s) improved by ΔR² = +0.171. PGA prediction R² increased from 0.516 to 0.696, and Sa(1.0 s) from 0.527 to 0.705. The near-doubling of V_S30 standard deviation (130.3 → 204.9 m/s) — reflecting the expanded range from 71.9 to 1949.3 m/s across NEHRP site classes A through E — enabled XGBoost to better resolve site amplification effects. These results unequivocally confirm that V_S30 enrichment is the single most impactful improvement pathway. Incorporating measured V_S30 from HVSR or borehole surveys at the remaining 516 BMKG stations without field measurements, combined with the site dominant period T_dom (Seyhan and Stewart, 2014), would yield further gains, particularly at short periods where site amplification effects are strongest.
