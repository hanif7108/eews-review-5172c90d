...
**Site effects.** The absence of site characterization parameters (V_S30, Z_2.5) is the dominant factor limiting short-period Sa predictions (T = 0.3–0.6 s, R² ≈ 0.54), as demonstrated by the 23% R² gap between short and long periods (Section 3.5). A preliminary experiment augmenting the feature set with slope-based V_S30 estimates (Allen and Wald, 2009) confirmed this hypothesis: retraining all 515 models with 44 features (42 P-wave IMs + V_S30 + ln V_S30) improved R² across all 103 spectral periods without exception, with a mean ΔR² = +0.030 (+5.1%). Short-period performance (T = 0.1–0.6 s) improved by ΔR² = +0.019 (+3.9%), while long-period performance (T ≥ 3.0 s) improved by ΔR² = +0.040 (+6.8%). The modest but universal improvement using approximate slope-based V_S30 suggests that incorporating measured V_S30 from field surveys or HVSR measurements — which capture actual site resonance characteristics including the dominant site period T_dom (Seyhan and Stewart, 2014) — would yield substantially larger gains, particularly at short periods where site amplification effects are strongest.
