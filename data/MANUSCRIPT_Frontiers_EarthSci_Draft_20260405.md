...
### 4.2 Distance Estimation Robustness and Self-Correcting PTW

A striking finding is the near-identical E2E performance between predicted and ground-truth (oracle) distance routing: R² = 0.7309 versus R² = 0.7311, a difference of merely ΔR² = 0.0002. This remarkable robustness holds despite the distance estimator's modest accuracy (R² ≈ 0.10, MAE ≈ 205 km), and arises from two architectural properties of the pipeline.

First, the distance serves only as a routing signal to select among discrete PTW windows, not as a continuous prediction input. Whether a trace is routed to PTW = 3 s or PTW = 4 s — both reasonable windows — the Stage 2 model at the assigned PTW has been trained to handle that window's specific information content. The penalty for misrouting is limited to the *difference* in R² between adjacent PTW models (~0.003–0.007), not the distance error itself. As shown in Table 6, the operational routing concentrates 99.9% of traces at PTW = 3–4 s, a conservative distribution that prioritizes warning timeliness.

Second, the IDA framework's intensity-driven routing already captures the dominant source of PTW variation (shaking severity), reducing the marginal contribution of distance to PTW selection. The golden time constraint primarily affects near-field events where the warning budget is tightest, and these events are rare in the predominantly far-field Sunda dataset (median distance 384 km, golden time ≈ 51 s).

This robustness simplifies operational deployment: **the pipeline requires zero external information from seismic networks or catalogs. A single station can autonomously determine the optimal balance between lead time and accuracy based solely on its own P-wave signal analysis.** The "Honest Evaluation" protocol confirms that the system maintains competitive accuracy under fully autonomous, single-station conditions.
...
**Class imbalance.** The *Damaging* class (PGA ≥ 62.0 Gal) comprises only 101 traces (0.3% of the dataset), which limits the internal generalizability of Stage 1 for the most extreme events. While we address this through class-weighted training and SMOTE-oversampling in Stage 1 training (Table S1), future iterations should incorporate synthetic seismic records or global transfer learning to populate the high-intensity regime.
...
**Site effects.** The absence of site characterization parameters (V_S30, Z_2.5) is the dominant factor limiting short-period Sa predictions (T = 0.3–0.6 s, R² ≈ 0.54), as demonstrated by the 23% R² gap between short and long periods (Section 3.5). A preliminary experiment incorporating V_S30 as an input feature confirmed this hypothesis: retraining all 515 models with 44 features (42 P-wave IMs + V_S30 + ln V_S30) improved R² across all 103 spectral periods without exception, with a mean ΔR² = +0.030 (+5.1%). Short-period performance (T = 0.1–0.6 s) improved by ΔR² = +0.019 (+3.9%), while long-period performance (T ≥ 3.0 s) improved by ΔR² = +0.040 (+6.8%). V_S30 values were sourced from BMKG station metadata (233 stations with measured values, ~30% of the dataset) supplemented by USGS global estimates (Allen and Wald, 2009) for stations without field measurements (~70%). The universal improvement — even with approximate V_S30 for the majority of stations — confirms that site characterization is the dominant limiting factor. Incorporating measured V_S30 from HVSR or borehole surveys at all BMKG stations, combined with the site dominant period T_dom (Seyhan and Stewart, 2014), would yield substantially larger gains, particularly at short periods where site amplification effects are strongest.
